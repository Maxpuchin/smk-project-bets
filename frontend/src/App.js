		...
		inputs: [
			{
				indexed: false,
				internalType: "bool",
				name: "_is_bet_finished",
				type: "bool",
			},
		],
		name: "betFinishedEvent",
		type: "event",
	},
	{
		anonymous: false,
...
function App() {
	...
	const onSubmit = () => {
		...
		myContract.methods
			.placeBet(selectedPlayer)
			.send({ from: walletAddress, gas: 380000, value: weiValue }, function (err, res) {
				console.log(err, res);
			});
	};
	...
	const onClickShowResults = () => {
		...
		myContract.methods
			.doesBetFinished()
			.send({ from: walletAddress, gas: 380000, value: 0 }, function (err, res) {
				console.log(err, res);
				if (!err) {
					myContract.events
						.betFinishedEvent()
						.on("data", (event) => {
							setIsBetFinished(event.returnValues._is_bet_finished);
							myContract.methods
								.doesGameFinished()
								.send({ from: walletAddress, gas: 380000, value: 0 }, function (err, res) {
									console.log(err, res);
									if (!err) {
										myContract.events
											.gameFinishedEvent()
											.on("data", (event) => {
												setIsGameFinished(event.returnValues._is_game_finished);
											})
											.on("error", console.error);
									} else {
										alert("Видимо, вы не поставили!");
									}
								});
						})
						.on("error", console.error);
				...
				<Card variant="outlined">
					<CardContent>
						...
						<Typography variant="body" marginY={1}>
							Игроки считают вместе вслух «Камень… Ножницы… Бумага…», одновременно качая кулаками.
							Затем каждый из двух игроков выбирает один из этих предметов. Важно, чтобы это было
							сделано одновременно. <br />
						...
						<Typography variant="button">
							<b>
								Вы можете поставить на первого или второго, выигрыш всех участников будет
								распределен пропорционально внесенным средствам.
							</b>
						</Typography>
						...
						<TextField
							...
							sx={{ my: 2 }}
						/>
						<Button variant="contained" color="success" fullWidth onClick={onSubmit}>
							Сделать ставку
						</Button>
						<Grid container sx={{ my: 4, width: "100%" }}>
							...
							<Grid item xs={6}>
								<Typography fullWidth>
									<b>Игра завершена:</b> {isGameFinished ? "Да" : "Нет"}
									<br />
									<b>Конкурс завершена:</b> {isBetFinished ? "Да" : "Нет"}
									<br />
								</Typography>
							</Grid>
						</Grid>
